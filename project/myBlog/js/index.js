window.onload = () => {
	...
	skillviewer = (skilltree, skillinfo) => {
		...
		$('.desk').css({
			width: '42rem',
			height: '42rem',
			transform: 'translate(42rem, -9rem)',
		});
		$('.desk').html(
			`<div class="infoskill">
			<h1>${skilltree}</h1>
			...
			button.addEventListener('click', (e) => {
				...
				switch (button.id) {
					...
					case 'node':
						skillviewer(
							`NODEjs`,
							`자바스크립트 런타임인 NOde.js를 이용하여 웹 브라우저 외부에서 자바스크립트 개발을 할 수 있습니다. 포트폴리오중에 키오스크를 노드 베이스로 개발했으며, 적극적으로 학습중입니다.`
						);
						break;
	...
	$('button').on('click', (e) => {
		switch (e.target.id) {
			case 'dogSleeppage':
				window.open('http://pager.kr/~c15st21/Portfolio/eventpage/');
				break;
			case 'dogSleepPpt':
				window.open('');
				break;
